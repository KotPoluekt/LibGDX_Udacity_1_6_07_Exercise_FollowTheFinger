...
public class BouncingBall extends InputAdapter {
    ...
    public void update(float delta) {
        ...
        if (targeting) {
            Vector2 difference = new Vector2(target.x - position.x, target.y - position.y);
            velocity.x = difference.x * FOLLOW_MULTIPLIER;
            velocity.y = difference.y * FOLLOW_MULTIPLIER;
        }
    ...
    public boolean touchDragged(int screenX, int screenY, int pointer) {
        ...
        if (targeting) {
            target = viewport.unproject(new Vector2(screenX, screenY));
        }
